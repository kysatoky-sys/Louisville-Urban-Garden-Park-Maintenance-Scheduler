"""
pipeline.py — Louisville Urban Garden & Park Maintenance Scheduler
------------------------------------------------------------------
Single-entry-point ETL pipeline. Runs three stages in sequence:

    EXTRACT   Read park coordinates from seed_parks.csv and fetch 16 days
              of hourly weather data from the Open-Meteo API for every park.

    TRANSFORM Aggregate hourly readings to daily values, validate and clean
              the data, and write data/weather_data.csv as an audit trail.

    LOAD      Read the seed CSVs and the weather CSV, evaluate each task's
              JSONB weather constraints to compute the maintenance schedule,
              create the database schema in Supabase, and insert all four
              tables: park, maintenance_task, weather_forecast,
              maintenance_schedule.

Inputs (data/):
    seed_parks.csv
    seed_maintenance_tasks.csv

Intermediate output written after Transform, read back during Load (data/):
    weather_data.csv

Loads into Supabase PostgreSQL (public schema):
    park
    maintenance_task
    weather_forecast
    maintenance_schedule
...
Optional .env overrides:
    SUPABASE_DB_URL=postgresql+psycopg2://...   full URL takes priority
    RESET_TABLES=true                           drop and recreate tables on each run
"""
...
# Hourly fields requested from the Open-Meteo API.
...
# Plausible daily value ranges for Louisville, KY — used for range validation.
# Soil temperature and soil moisture are daily means;
# evapotranspiration and precipitation are daily totals.
...
# ---------------------------------------------------------------------------
# Logging — single shared logger for the whole pipeline
# ---------------------------------------------------------------------------
...
# The IDs are written into weather_data.csv and the load stage reads them
# back from that file — it does not re-derive them independently. This means
# the IDs are consistent even if seed_parks.csv is reordered, because the
# load stage always follows what the extract stage produced rather than
...
# Fetches hourly weather data for a single park and returns a tidy DataFrame.
# The openmeteo-requests client handles caching and retries automatically.
# Returns None if the API call or response parsing fails so the caller can
# skip this park and continue with the others.
...
# Soil temperature and soil moisture are averaged (point measurements).
# Evapotranspiration and precipitation are summed (accumulations).
...
# Applies four data quality checks to the aggregated daily DataFrame
...
# the load stage to run but produce an empty database with no error surfaced.
...
# Reads credentials from a .env file and supports a direct SUPABASE_DB_URL override.
...
# Defines the four tables and their relationships.
# Optionally drops existing tables and recreates the schema from scratch.
...
# that the extract stage stamped onto that park's rows in weather_data.csv.
#
...
#
# Why this matters: the extract stage assigns park_id by row position
# (index + 1). If the load stage re-derived IDs independently by row
# position, a reordered or edited CSV would cause weather data for one park
# to be stored under a different park's ID — silently, with no error.
#
# park_id is used during schedule computation and as the FK when inserting
# weather_forecast and maintenance_schedule rows. It is dropped before the
# park table insert so PostgreSQL's SERIAL column auto-assigns the same values.
def read_parks(weather_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # The extract stage wrote park_id into every weather row, so this lookup
    # is the authoritative source of which ID belongs to which park.
    id_lookup = (
...
def read_tasks() -> pd.DataFrame:
    ...
    # Validation check 6 — Schema/type validation on weather_constraints
...
# Reads weather_data.csv written by the transform stage.
...
# ===========================================================================
# MAIN — orchestrate all three stages
# ===========================================================================
...
def main() -> None:
    ...
    # Write intermediate CSV — audit trail and safety net if load stage fails
    ...
    # Read weather_data.csv back from disk so park IDs are sourced from the
    # file — the same authoritative path used when load.py ran standalone.
    ...
    logger.info("Creating Supabase PostgreSQL schema...")
    create_schema(engine)

    # Load parent tables first — no foreign key dependencies.
    # park_id and task_id are dropped so PostgreSQL's SERIAL columns
    # auto-assign the same values.
    write_table(
    ...
    logger.info("  Parks processed    : %d", len(parks_df))
    logger.info("  Forecast rows      : %d", len(weather_df))
    logger.info("  Schedule rows      : %d", len(schedule_df))
    logger.info("  Log file           : %s", LOG_DIR / "etl.log")
    logger.info("=" * 60)
